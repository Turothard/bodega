            ...
            <div class="col-8">
                ...
                <tbody>
                    <tr v-for="(item,index) in detalleinforme" :key="index">
                        <td>{{item.fechaoc}}</td>
                        <td>{{item.nrooc}}</td>
                        <td>{{ proveedores.find( items => items.rutproveedor === item.proveedor_id ).nombreprov }}</td>
                        <td>{{item.estadooc}}</td>
                        <td>{{item.montooc}}</td>
                    </tr>
                </tbody>
        ...
        // a computed getter
         ...
         methods: {
             ...
             filtrar(){
                ...
                axios.post('/informes/generar', {tipo:'informefrecuenciaoc',detalle: this.filtros})
                    .then((res) =>{
             ...
             descargarinforme(){
                 console.log(this.filtros);
                axios.post('/informes/export', {tipo:'informefrecuenciaoc',detalle: this.filtros, arreglo:this.detalleinforme})
                    .then((res) =>{
                    setTimeout(function() {
